...
const colombiaLat = () =>
  faker.number.float({ min: 1.5, max: 12.5, fractionDigits: 6 });

const colombiaLng = () =>
  faker.number.float({ min: -79.0, max: -66.8, fractionDigits: 6 });
...
const vias = [
  ...
  "Autopista Barranquilla-Cartagena",
  "Ruta 25",
...
const createCamaraVelocidad = () => {
  ...
  return {
    ...
    limite_velocidad: faker.helpers.arrayElement([
      30, 40, 50, 60, 80, 100, 120,
    ]),
    estado: faker.helpers.arrayElement(estados),

    // IMPORTANTE: el test espera "coords"
    coords: {
      lat: colombiaLat(),
      lng: colombiaLng(),
    },

    fecha_instalacion: faker.date
      .between({
        from: "2015-01-01",
        to: "2024-12-31",
      })
...
// FUNCIÓN QUE EL TEST NECESITA
const generateAdventures = (total = 1) => {
  return faker.helpers.multiple(createCamaraVelocidad, {
    count: total,
  });
};

// SOLO imprime si ejecutas el archivo directamente
if (require.main === module) {
  const camaras = generateAdventures(500);
